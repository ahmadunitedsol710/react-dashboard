    ...
    const add_fav = (data) => {
        ...
        if(favdata.length < 1) {
            ...
            if (isFav) {
                ...
                // localStorage.setItem("array", JSON.stringify(favdata))
                // localStorage.removeItem("array", JSON.stringify(favdata))
                console.log("else 1")
            } 
            else {
                            ...
                            <table>
                                <tbody>
                                    {showfav ? tabledata.map((data)=>(
                                        ...
                                        <td className='details' data-label="date">{data?.date}</td>
                                        <td className='details' data-label="location">{data?.location}</td>
                                        <td className='details' data-label="location"> <img src={favdata.some(favitem=>favitem.id === data.id) ? emptyfav : emptyfav} alt="favourite" 
                                        onClick={()=>add_fav(data)}/></td>
                                    </tr> 
                                    ))}
                                </tbody>
