...
const Eventlist = ({showfav}) => {
    ...
    const add_fav = (data) => {
        // e.preventDefault();
    // Retrieve the existing array from localStorage, or initialize an empty array if it doesn't exist
    let favitemsarray = JSON.parse(localStorage.getItem("favicon")) || [];

    // Add the new data item to the array
    favitemsarray.push(data);

    // Save the updated array back to localStorage
    localStorage.setItem("favicon", JSON.stringify(favitemsarray));
    setIsFaviconSet(prevState => !prevState);

    // Optional: Log the updated array to the console for debugging
    console.log(favitemsarray);
