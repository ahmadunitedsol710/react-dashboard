...
const Eventlist = ({showfav}) => {
   ...
   let itemIndex = favitemsarray.findIndex(item => item.id === data.id);

   if (itemIndex !== -1) {
       // If item exists, toggle its state
       favitemsarray[itemIndex].isFav = !favitemsarray[itemIndex].isFav;
   } else {
       // If item does not exist, add it with isFav set to true
       data.isFav = true;
