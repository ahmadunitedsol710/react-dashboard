...
const Upcommingevent = (eventdata, setStoredData) => {
    const add_fav = (data) => {
        const isFav = eventdata.some(favitem => favitem.id === data.id)

        if (isFav) {
            setStoredData(eventdata.filter(favitem => favitem.id !== data.id));    
            // localStorage.setItem("not favourite", JSON.stringify(favdata))
            localStorage.removeItem(JSON.stringify(eventdata))
            console.log("else 1")
        } 
        else {
            // Add the item if it does not exist
            setStoredData(prevFavarr => [...prevFavarr, data]);
            localStorage.setItem("favourite", JSON.stringify(eventdata))
            console.log("else 2")
        }
    };
                ...
                <ul className="events_list">
                    {eventdata.map((list) => {
                        ...
                        <li className="event" key={list.id}>
                            ...
                            <div>
                                <img src={eventdata.some(favitem => favitem.id === list.id) ? fillfav : emptyfav} alt="emptyfav" 
